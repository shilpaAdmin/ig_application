    ...
    public function storeCommentReplyData(Request $request)
    {
        ...
        if($obj->save())
        {
            if(isset($input['CommentID']) && !empty($input['CommentID']))
            {
                ...
                $result=app('App\Http\Controllers\Api\NotificationsController')->sendNotification($blogCommentObj->user_id,$input['BlogId'],$user->name.' added reply on your comment.','blog_reply','','');
            }
            else
            {
                if(!empty($blog->user_id) && $blog->user_id > 0 )
                $result=app('App\Http\Controllers\Api\NotificationsController')->sendNotification($blog->user_id,$input['BlogId'],$user->name.' added comment in your blog.','blog_comment','','');
            }
