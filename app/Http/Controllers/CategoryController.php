...
class CategoryController extends Controller
{
    ...
    public function subCategoryDataList(Request $request)
    {
        ...
        return DataTables::of($result_obj)->addColumn('applicant', function ($result_obj) {
            ...
            ->rawColumns(['name', 'description', 'resume_cv', 'media_file', 'status_td', 'jobapplicant', 'applicant', 'id'])
            ->make(true);
    }
    ...
    public function businessListapplicant(Request $request)
    {
        ...
        return DataTables::of($result_obj)
            ...
            ->addColumn('jobapplicant', function ($result_obj) {
                // dd($result_obj);
                $jobapplicant = '';
                return '<a href="javascript:;" onclick="openJobapplicantPopup(' . $result_obj['id'] . ')" class="undar_line desc"  data-toggle="modal" data-target=".bs-example-modal-center" class="btn btn-primary waves-effect btn-label waves-light">Applicant</a>';
            })
            ->addColumn('status_td', function ($result_obj) {
                $status = '';
                if ($result_obj->status == 'active')
            ...
            ->rawColumns(['DT_RowId', 'status_td', 'command', 'image_src','jobapplicant', 'type_td', 'name_td'])
            ->make(true);
    }
